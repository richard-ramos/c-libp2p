...
/* conn_pipeline_new: allocates a connection for the upgrade pipeline.
 * Called externally via lp2p_conn_attach_tcp + lp2p_conn_upgrade. */
static lp2p_conn_t * __attribute__((unused))
conn_pipeline_new(uv_loop_t *loop, bool is_inbound, void *host_ptr) {
...
static void conn_async_close_cb(uv_handle_t *handle) {
    (void)handle;
    /* Nothing to free — the async handle is embedded in lp2p_conn_t */
}
...
static void __attribute__((unused)) conn_pipeline_free(lp2p_conn_t *conn) {
    if (!conn) return;
...
static void conn_on_mux_inbound_stream(yamux_session_t *session,
    ...
    /* Route to the appropriate protocol handler via the router */
    if (conn->router && stream->protocol_id) {
        lp2p_list_node_t *cur = conn->router->entries.head.next;
        lp2p_list_node_t *end = &conn->router->entries.head;
        while (cur != end) {
            conn_proto_entry_t *entry =
                lp2p_container_of(cur, conn_proto_entry_t, node);
            if (strcmp(entry->protocol_id, stream->protocol_id) == 0) {
                entry->handler(stream, entry->userdata);
                return;
            }
            cur = cur->next;
        }
...
static void conn_become_ready(lp2p_conn_t *conn) {
    ...
    while (!lp2p_list_empty(&conn->pending_streams)) {
        ...
        lp2p_err_t err = conn->mux->vtable->open_stream(conn->mux->impl,
                                                          &stream);
        if (err == LP2P_OK && stream) {
            stream->conn = conn;
            stream->protocol_id = req->protocol_id;
            req->protocol_id = NULL; /* ownership transferred */
        }
...
/* NOTE: The public API functions lp2p_conn_open_stream, lp2p_conn_close,
 * lp2p_conn_peer_id, lp2p_conn_remote_addr, lp2p_conn_local_addr, and
 * lp2p_conn_is_inbound are defined in connmgr.c, which owns the lp2p_conn_t
 * struct layout used by the rest of the system. The upgrade pipeline in this
 * file is an internal implementation detail called by the dialer/listener. */
