...
static void host_on_inbound_conn(lp2p_listener_t *listener, lp2p_conn_t *conn,
                                   void *userdata);
...
lp2p_err_t lp2p_host_listen(lp2p_host_t *host, lp2p_on_listen_cb cb,
    ...
    for (size_t i = 0; i < host->listen_addrs_count; i++) {
        ...
        err = lp2p_listener_start(listener, host_on_inbound_conn, host);
        if (err != LP2P_OK) {
...
static void dial_done(lp2p_conn_t *conn, lp2p_err_t err, void *userdata) {
    ...
    if (ctx->has_expected_peer && conn->peer_id_known) {
        if (!lp2p_peer_id_equal(&conn->peer_id, &ctx->expected_peer)) {
            lp2p_conn_close(conn, NULL, NULL);
    ...
    if (add_err == LP2P_ERR_ALREADY_CONNECTED) {
        /* Check tiebreak */
        lp2p_conn_t *existing = lp2p_connmgr_get(ctx->host->connmgr, &conn->peer_id);
        if (existing) {
            ...
            lp2p_conn_close(existing, NULL, NULL);
...
lp2p_err_t lp2p_host_dial(lp2p_host_t *host, const char *multiaddr,
    ...
    /* Dial via the dialer */
    err = lp2p_dialer_dial(host->dialer, ma, dial_done, ctx);
    lp2p_multiaddr_free(ma);
...
static void new_stream_dial_done(lp2p_conn_t *conn, lp2p_err_t err,
    ...
    lp2p_err_t serr = lp2p_conn_open_stream(conn, ctx->protocol_id, ctx->stream_cb, NULL);
    if (serr != LP2P_OK) {
...
static void host_on_inbound_conn(lp2p_listener_t *listener, lp2p_conn_t *conn,
    ...
    if (!conn || host->closing) {
        if (conn) lp2p_conn_close(conn, NULL, NULL);
        return;
    }
    ...
    if (err == LP2P_ERR_ALREADY_CONNECTED && conn->peer_id_known) {
        lp2p_conn_t *existing = lp2p_connmgr_get(host->connmgr, &conn->peer_id);
        if (existing) {
