...
static void dial_ctx_retain(dial_ctx_t *ctx)
{
    if (ctx) ctx->refs++;
}

static void dial_ctx_release(dial_ctx_t *ctx)
{
    if (ctx && ctx->refs > 0 && --ctx->refs == 0) {
        free(ctx);
    }
}

static void dial_timer_close_cb(uv_handle_t *handle)
{
    dial_ctx_t *ctx = (dial_ctx_t *)handle->data;
    dial_ctx_release(ctx);
}

static void dial_ctx_close_timer(dial_ctx_t *ctx)
{
    if (!ctx || ctx->timer_closing) return;

    ctx->timer_closing = true;
    uv_timer_stop(&ctx->timer);
    uv_close((uv_handle_t *)&ctx->timer, dial_timer_close_cb);
}
                              ...
                              void *userdata)
{
    ...
    /* Start timeout timer */
    if (uv_timer_init(dialer->loop, &ctx->timer) != 0) {
        free(ctx);
        return LP2P_ERR_TRANSPORT;
    }
    ctx->timer.data = ctx;
