                        ...
                        lp2p_err_t err, void *userdata)
{
    ...
    /* Preserve the decoded frame before consume() shifts the TCP buffer. */
    ...
    case MS_STATE_RECV_PROPOSAL: {
        ...
        if (found) {
            memcpy(neg->negotiated_proto, proto, copy_len);
            neg->negotiated_proto[copy_len] = '\0';
    ...
    case MS_STATE_RECV_ACCEPT: {
        ...
        if (msg_len == 2 && memcmp(msg_copy, MULTISTREAM_NA, 2) == 0) {
            neg->state = MS_STATE_FAILED;
        ...
        if (msg_len == proto_len &&
            memcmp(msg_copy, neg->proposed_proto, msg_len) == 0) {
            memcpy(neg->negotiated_proto, neg->proposed_proto, proto_len);
            neg->negotiated_proto[proto_len] = '\0';
...
static void ms_drive(ms_ctx_t *ctx)
{
    ...
    case MS_STATE_RECV_ACCEPT:
        ...
        if (lp2p_tcp_conn_start_read(ctx->tc, ms_on_read, ctx) != LP2P_OK) {
            neg->state = MS_STATE_FAILED;
            ctx->cb(LP2P_ERR_NEGOTIATION_FAILED, NULL, ctx->userdata);
            free(ctx);
            return;
